...
from authorization import ping_oauth_link, ping_oauth_pin
...
"}")
    ...
    def create_account(self):
        ...
        self.acct_window = QtWidgets.QMainWindow()
        window = self.acct_window
        
        self.acct_window.resize(200,300)
        ...
        # self.acct_accept_button.clicked.connect(self.auth_url)
        
        self.acct_cancel_button = QtWidgets.QPushButton("Cancel")
        self.acct_cancel_button.clicked.connect(lambda: self.close_window(window))
    ...
    def create_account_accept(self):
        ...
        if self.check_acct_fields():
            ...
            self.db.create_account(self.acct_id_edit.text(), self.acct_pass_edit.text(), self.acct_name_edit.text())
            
            self.congrats_widget = QtWidgets.QWidget(self)
            self.congrats_layout = QtWidgets.QVBoxLayout()
            self.congrats_widget.setLayout(self.congrats_layout)
            
            self.congrats_label0 = QtWidgets.QLabel("Congratulations!")
            self.congrats_label0.setAlignment(Qt.AlignCenter)
            
            self.congrats_label1 = QtWidgets.QLabel(new_acct_prompt)
            #self.congrats_label2 = QtWidgets.QLabel()
            
            self.congrats_buttons_widget = QtWidgets.QWidget()
            self.congrats_buttons_widget_layout = QtWidgets.QHBoxLayout()
            self.congrats_buttons_widget.setLayout(self.congrats_buttons_widget_layout)
            self.congrats_buttons_widget_layout.setAlignment(Qt.AlignCenter)
            self.congrats_button_alright = QtWidgets.QPushButton("Let's Go!")
            self.congrats_button_alright.clicked.connect(lambda: self.close_window(window))
            self.congrats_button_alright.clicked.connect(self.auth_pin)
            

            
            self.congrats_buttons_widget_layout.addWidget(self.congrats_button_alright)
            
            self.congrats_pin_edit = QtWidgets.QLineEdit()
            self.congrats_pin_edit.setPlaceholderText("Enter PIN")
            
            self.congrats_layout.addWidget(self.congrats_label0)
            self.congrats_layout.addWidget(self.congrats_label1)
            self.congrats_layout.addWidget(self.congrats_pin_edit)
            self.congrats_layout.addWidget(self.congrats_buttons_widget)
            
            self.acct_window.setCentralWidget(self.congrats_widget)
            
    
    def check_acct_fields(self):
        # pattern = "[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}"
        user_id = self.acct_id_edit.text()
        user_name = self.acct_name_edit.text()
        user_pass = self.acct_pass_edit.text()
        user_confirm_pass = self.acct_confirm_pass_edit.text()
        ...
        elif(user_confirm_pass ==""):
            return 1
            print(text)
            
        elif (self.db.user_exists(user_id)):
            print("user already exists")
            return 1
        
        elif(self.acct_pass_edit.text() != self.acct_confirm_pass_edit.text()):
            print("non-matching passwords")
            return 1
            #need pop up
            
        # elif(re.search(pattern, self.acct_id_edit.text()) == None):
        #     print("bad email")
    ...
    @pyqtSlot()
    def auth_url(self):
        QDesktopServices.openUrl(QUrl(ping_oauth_link()))
    
    @pyqtSlot()
    def auth_pin(self):
        ping_oauth_pin(self.congrats_pin_edit.text().replace(" ",""))
...
############################################################
# Some general prompts for windows

new_acct_prompt = "You have just created a new Pinguin account. Enter the PIN provided by the link:"        