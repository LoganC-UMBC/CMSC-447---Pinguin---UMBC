...
from ..Functions.Google_Drive.gDrive import *

#Google Calendar
from ..Functions.Google_Calendar.gcalendar import *
...
########################################################################################################################
    ...
    def delete_group(self):
        ...
        if index.data() == None:
            ...
            if self.db.check_ownership(item.group_id, self.db.user._id):
                self.db.delete_group(item.group_id)
    ...
    def remove_user(self):
        ...
        if index.data() == None:
            ...
            elif item.role == "owner" and item.group_id == str(self.db.user._id):
                print("You can't remove yourself")
                error_text = "You can't remove yourself from the group"
                self.groups_error_label.setText(error_text)
                self.error_frame_show(self.groups_error_frame)

            elif item.role == "owner" and item.parent().group_id != self.current_group:
                error_text = "You are not the owner"
                self.groups_error_label.setText(error_text)
                self.error_frame_show(self.groups_error_frame)
...
########################################################################################################################
    ...
    def add_event(self):
        ...
        start_time = datetime.datetime(event_year,event_month,event_day,0,0,0,0,pytz.timezone('US/Eastern'))
        end_time = datetime.datetime(event_year, event_month, event_day, 23, 59, 0, 0, pytz.timezone('US/Eastern'))
